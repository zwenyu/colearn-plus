...
def split_target(args):
    ...
    test_set = ImageList(txt_tar, transform_adapt=test_transform)
    ...
    return new_src.copy(), new_tar.copy(), endtime-starttime
...
def data_load(args, txt_src, txt_tgt):
    ...
    dsets = {}
    dsets["source"] = ImageList(txt_src, transform_adapt=train_transform)
    dsets["target"] = ImageList_twice(txt_tgt, transform=[train_transform, train_transform])

    txt_test = open(args.test_dset_path).readlines()
    dsets["test"] = ImageList(txt_test, transform_adapt=test_transform)
...
def train(args, txt_src, txt_tgt):
    ...
    for iter_num in range(1, max_iter + 1):
        ...
        try:
            data_source = source_loader_iter.next()
        except:
            source_loader_iter = iter(dset_loaders["source"])
            data_source = source_loader_iter.next()
        inputs_source = data_source[0]
        labels_source = data_source[1]

        try:
            data_target = target_loader_iter.next()
        except:
            target_loader_iter = iter(dset_loaders["target"])
            data_target = target_loader_iter.next()
        inputs_target = data_target[0]
        labels_target = data_target[2]
    ...
    if args.issave:   
        torch.save(netG.state_dict(), osp.join(args.mm_dir, "target_F_" + "{:}_{:}".format(args.log, args.choice) + ".pt"))
        torch.save(netB.state_dict(), osp.join(args.mm_dir, "target_B_" + "{:}_{:}".format(args.log, args.choice) + ".pt"))
        torch.save(netC.state_dict(), osp.join(args.mm_dir, "target_C_" + "{:}_{:}".format(args.log, args.choice) + ".pt"))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='office-home', choices=['VISDA-C', 'office', 'office-home', 'domainnet-126'], help="The dataset or source dataset used")
    ...
    for i in range(len(names)):
        ...
        if args.model == "source": 
            args.output_dir = osp.join(args.output_tar, args.da, args.dset, names[args.s], str(SEED))
        else:
            args.output_dir = osp.join(args.output_tar, args.da, args.dset, names[args.s]+'_'+names[args.t], str(SEED))
        ...
        args.mm_dir = osp.join(args.output, args.da, args.dset, names[args.s]+'_'+names[args.t], str(SEED))
        ...
        dir_list = os.listdir(args.mm_dir)
        if ("target_F_" + "{:}_{:}".format(args.log, args.choice) + ".pt") in dir_list:
            print('Experiment already completed.')
        else:
            args.out_file = open(osp.join(args.mm_dir, "{:}_{:}.txt".format(args.log, args.choice)), "w")

            utils.print_args(args)
            txt_src, txt_tgt, split_time = split_target(args)
            results_dict = train(args, txt_src, txt_tgt)
            results_dict['split_time'] = split_time

            args.out_file.close()

            # save results to json file
            args.time_json = os.path.join(args.mm_dir, f'time_{args.choice}.jsonl')
