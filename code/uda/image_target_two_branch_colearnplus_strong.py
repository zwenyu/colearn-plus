...
@torch.no_grad()
def get_pred_conf(args, loader, netF, netB, netC, netP, netC_text=None, model_type=None):
    '''
    ...
    for _ in range(len(loader)):
        ...
        if model_type == 'adapt':
            ...
            if netB is not None:
                feas = netB(feas)
            logits_text = netC_text(feas) * 0.05  # temperature applied for text classifier
...
def data_load_pred(args, dset_loaders, adapt_netF, adapt_netB, adapt_netC,
    pretrained_netF, pretrained_netB, pretrained_netC, pretrained_netP, pretrained_netC_text, branch):
    '''
    ...
    pretrained_preds, pretrained_ind_confs = get_pred_conf(args, dset_loaders['target_all'],
        pretrained_netF, pretrained_netB, pretrained_netC, pretrained_netP, pretrained_netC_text, model_type='pretrained')
    # prediction and confidence by adaptation model
    adapt_preds, adapt_ind_confs = get_pred_conf(args, dset_loaders['target_all'],
        adapt_netF, adapt_netB, adapt_netC, None, model_type='adapt')
...
def finetune(args, dset_loaders, adapt_netF, adapt_netB, adapt_netC, pretrained_netF):
    ...
    centroids = method_utils.get_centroids(args, dset_loaders['target_all'], adapt_netF, adapt_netB, adapt_netC, pretrained_netF, None, pretrained_prototype_netC,
        use_source_pseudolabel=True, model_type='adapt', temperature=None)
    ...
    pretrained_acc_init, pretrained_accmean_init, _ = eval_utils.cal_acc(dset_loaders['target_all'], 
        pretrained_netF, None, {'netC': pretrained_netC, 'prototype_netC': pretrained_prototype_netC}, pretrained_netP, 'NCC_mixed', model_type='pretrained')
    ...
    for epoch in range(args.max_epoch):
        ...
        dset_loaders, adapt_ind_confs, pretrained_ind_confs, ind_pseudolabel = data_load_pred(args, dset_loaders, 
            adapt_netF, adapt_netB, adapt_netC, pretrained_netF, None, pretrained_netC, pretrained_netP, pretrained_prototype_netC, branch='adapt')
        ...
        dset_loaders, _, _, _ = data_load_pred(args, dset_loaders, adapt_netF, adapt_netB, adapt_netC, 
            pretrained_netF, None, pretrained_netC, pretrained_netP, pretrained_prototype_netC, branch='pretrained')
        max_iter = len(dset_loaders['target_pred_train'])
    ...
    return adapt_netF, results_init
...
def train_target(args):
    ...
    # finetuning models
    starttime = time.time()
    adapt_netF, results_init = finetune(args, dset_loaders, adapt_netF, adapt_netB, adapt_netC, pretrained_netF)
    endtime = time.time()
    ...
    results_dict = {**results_init, **acc_consolidate, **acc_per_class, 'time': endtime - starttime}
...
if __name__ == '__main__':
    ...
    for i in range(len(names)):
        ...
        if args.pretrained_model_type == 'source':
            ...
            args.final_output_dir = osp.join(args.expt_name, args.output, args.da, args.dset, names[args.s] + '_' + names[args.t], str(SEED))
        elif args.pretrained_model_type == 'targetpretrained':
            raise NotImplementedError
